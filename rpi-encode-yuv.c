...
int main(int argc, char **argv) {
	...
	// BUSY-While을 하면서, 입력이 Empty 하면, 채워넣고, Output이 찼으면 꺼내고 함. 
	// 만약에 CAMERA 입력을 추가로 사용한다면.  while 안에 flag를 하나 더 둬서
    ...
    while(1) {
        ...
        if(ctx.encoder_input_buffer_needed   // 버퍼가 비었음. 즉, busy-while? 
		     && input_available) {           // 아직 영상이 남아 있음.
            input_total_read = 0;
            ...
            for(i = 0; i < 3; i++) {
				...
				// not enough data in file, so stop encoding 
        ...
        if(want_quit && frame_out == frame_in) {  // 모든 프레임을 마무리했음.
            break;
        }
