...
typedef struct {
    OMX_HANDLETYPE encoder;  // 1st encoder
    OMX_HANDLETYPE encoder2; // 2nd encoder
    OMX_BUFFERHEADERTYPE *encoder_ppBuffer_in;
    OMX_BUFFERHEADERTYPE *encoder_ppBuffer_out;
    OMX_BUFFERHEADERTYPE *encoder_ppBuffer_in2;
    OMX_BUFFERHEADERTYPE *encoder_ppBuffer_out2;
    int encoder_input_buffer_needed;
    int encoder_output_buffer_available;
    int encoder_input_buffer_needed2;
    int encoder_output_buffer_available2;
    int flushed; // we can share this flag among components 
    FILE *fd_in;
    FILE *fd_out;
    FILE *fd_out2;
...
// fill a frame info strcture from w,h,stride, slice height 
// Stolen from video-info.c of gstreamer-plugins-base
#define ROUND_UP_2(num) (((num)+1)&~1)
#define ROUND_UP_4(num) (((num)+3)&~3)
static void get_i420_frame_info(int width, int height,
                                int buf_stride, int buf_slice_height,
...
// wait until the port state changed  
...
static OMX_ERRORTYPE empty_input_buffer_done_handler(
    ...
    if(hComponent == ctx->encoder)  // 1st encoder
       ctx->encoder_input_buffer_needed = 1;
    else if(hComponent == ctx->encoder2)  // 1st encoder
       ctx->encoder_input_buffer_needed2 = 1;
...
/*----------------------------------------------------------
    ...
    can we have simple OMX or multiple core for multi-encoders 
...
int main(int argc, char **argv) {
    ...
    // 2nd encoder
    say("Default port definition for encoder input port 200");
    dump_port(ctx.encoder2, 200, OMX_TRUE);
    say("Default port definition for encoder output port 201");
    dump_port(ctx.encoder2, 201, OMX_TRUE);
    ...
    // 2nd 
    encoder_portdef.nPortIndex = 200;
    if((r = OMX_GetParameter(ctx.encoder2, OMX_IndexParamPortDefinition, &encoder_portdef)) != OMX_ErrorNone) {
        omx_die(r, "Failed to get port definition for encoder input port 200");
    }
    encoder_portdef.format.video.nFrameWidth  = VIDEO_WIDTH;
    encoder_portdef.format.video.nFrameHeight = VIDEO_HEIGHT;
    ...
    encoder_portdef.format.video.nStride      = (encoder_portdef.format.video.nFrameWidth + encoder_portdef.nBufferAlignment - 1) & (~(encoder_portdef.nBufferAlignment - 1));
    encoder_portdef.format.video.eColorFormat = OMX_COLOR_FormatYUV420PackedPlanar;
    if((r = OMX_SetParameter(ctx.encoder2, OMX_IndexParamPortDefinition, &encoder_portdef)) != OMX_ErrorNone) {
        omx_die(r, "Failed to set port definition for encoder input port 200");
    }
    ...
    // 2nd
    if((r = OMX_SetParameter(ctx.encoder2, OMX_IndexParamVideoBitrate, &bitrate)) != OMX_ErrorNone) {
        omx_die(r, "Failed to set bitrate for encoder output port 201");
    }
    // Configure format
    ...
    // 2nd
    if((r = OMX_SetParameter(ctx.encoder2, OMX_IndexParamVideoPortFormat, &format)) != OMX_ErrorNone) {
        omx_die(r, "Failed to set video format for encoder output port 201");
    }
    ...
    // 2nd
    if((r = OMX_SendCommand(ctx.encoder2, OMX_CommandStateSet, OMX_StateIdle, NULL)) != OMX_ErrorNone) {
        omx_die(r, "Failed to switch state of the encoder component to idle");
    }
    block_until_state_changed(ctx.encoder2, OMX_StateIdle);
    ...
    // 2nd 
    if((r = OMX_SendCommand(ctx.encoder2, OMX_CommandPortEnable, 200, NULL)) != OMX_ErrorNone) {
        omx_die(r, "Failed to enable encoder input port 200");
    }
    block_until_port_changed(ctx.encoder2, 200, OMX_TRUE);
    if((r = OMX_SendCommand(ctx.encoder2, OMX_CommandPortEnable, 201, NULL)) != OMX_ErrorNone) {
        omx_die(r, "Failed to enable encoder output port 201");
    }
    block_until_port_changed(ctx.encoder2, 201, OMX_TRUE);
    ...
    if((r = OMX_AllocateBuffer(ctx.encoder2, &ctx.encoder_ppBuffer_in2, 200, NULL, encoder_portdef.nBufferSize)) != OMX_ErrorNone) {
        omx_die(r, "Failed to allocate buffer for encoder2 input port 200");
    }
    ...
    if((r = OMX_AllocateBuffer(ctx.encoder2, &ctx.encoder_ppBuffer_out2, 201, NULL, encoder_portdef.nBufferSize)) != OMX_ErrorNone) {
        omx_die(r, "Failed to allocate buffer for encoder output port 201");
    }
    ...
    ctx.fd_in = stdin;      // 화일명 사용하는 것이 좋을 듯? 
    
    //ctx.fd_out = stdout;    // 화일명 사용하는 것이 좋을 듯? 
    ctx.fd_out = fopen("test1.h264", "wb"); 
    if(!ctx.fd_out) omx_die(r, "cannot open out file 1");
    ctx.fd_out2 = fopen("test2.h264", "wb"); 
    if(!ctx.fd_out2) omx_die(r, "cannot open out file 2");
    ...
    // 2st
    if((r = OMX_SendCommand(ctx.encoder2, OMX_CommandStateSet, OMX_StateExecuting, NULL)) != OMX_ErrorNone) {
        omx_die(r, "Failed to switch state of the encoder component to executing");
    }
    block_until_state_changed(ctx.encoder2, OMX_StateExecuting);
    ...
    // fill the frame structure parameters
    ...
    dump_frame_info("Destination frame", &frame_info); // destination?
    dump_frame_info("Source buffer", &buf_info);

    // 1st 
    if(ctx.encoder_ppBuffer_in->nAllocLen != buf_info.size) {
        die("Allocated encoder 1 input port 200 buffer size %d doesn't equal to the expected buffer size %d", ctx.encoder_ppBuffer_in->nAllocLen, buf_info.size);
    }
    // 2nd
    if(ctx.encoder_ppBuffer_in2->nAllocLen != buf_info.size) {
        die("Allocated encoder 2 input port 200 buffer size %d doesn't equal to the expected buffer size %d", ctx.encoder_ppBuffer_in2->nAllocLen, buf_info.size);
    }

    say("Enter encode loop, press Ctrl-C to quit...");

    int input_available = 1, frame_in = 0, frame_out = 0, frame_out2 = 0, i;
    ...
    ctx.encoder_input_buffer_needed = 1;   // 초기화 
    ctx.encoder_input_buffer_needed2 = 1;   // 초기화 
    ...
    // busy while loop
    //   if(camera_out_available && encoder_buffer_empty)
    //         두버퍼 사이를 복사
    //         우선 저장을 하고, 나중에는 이를 사이즈를 줄여서 FFMPEG으로 압축후 보냄.  
    //   if(encoder_out_available)
	//         인코딩 결과를 저장/네트워크로 보냄
	//
    while(1) {
        ...
        if(ctx.encoder_input_buffer_needed 
           && ctx.encoder_input_buffer_needed2   // omx encoder input ports 
		     && input_available) {           
            input_total_read = 0;
            memset(ctx.encoder_ppBuffer_in->pBuffer, 0, ctx.encoder_ppBuffer_in->nAllocLen);
            memset(ctx.encoder_ppBuffer_in2->pBuffer, 0, ctx.encoder_ppBuffer_in2->nAllocLen);
            
			// YUV 순서로 화일에서 읽어서 버퍼에 채움.
			// Pack Y, U, and V plane spans read from input file to the buffer
            for(i = 0; i < 3; i++) {
                // data sizes for y, u, v of one frame
				want_read = frame_info.p_stride[i] * (i == 0 ? plane_span_y : plane_span_uv);
                // read as much as 'want=needed'
				// 1st
				input_read = fread(
                    ctx.encoder_ppBuffer_in->pBuffer + buf_info.p_offset[i],
                    1, want_read, ctx.fd_in);
                                // 2nd
                                memcpy(ctx.encoder_ppBuffer_in2->pBuffer + buf_info.p_offset[i], 
                                       ctx.encoder_ppBuffer_in->pBuffer + buf_info.p_offset[i], 
                                       want_read);
                               
                input_total_read += input_read;
                if(input_read != want_read) { //  no more data in file 
                    ctx.encoder_ppBuffer_in->nFlags = OMX_BUFFERFLAG_EOS;
                    ctx.encoder_ppBuffer_in2->nFlags = OMX_BUFFERFLAG_EOS;
                    want_quit = 1;
                    say("Input file EOF");
                    break;  // 안쪽 for 루프만 빠져가나므로 want_quit=1이 필요함.
					        // 이런 경우는 GOTO가 나을 듯.  
                }
            }
	    // 1st 
            ...
            // 2nd
            ctx.encoder_ppBuffer_in2->nOffset = 0;
            ctx.encoder_ppBuffer_in2->nFilledLen = (buf_info.size - frame_info.size) + input_total_read;
            frame_in++;
            ...
            if(input_total_read > 0) {  // want_quit인 경우도 이걸 해야하나?
				// 아마도 이함수는 비동기적으로 동작. 
                // 1st 
		...
		// 2nd
                ctx.encoder_input_buffer_needed2 = 0;
                if((r = OMX_EmptyThisBuffer(ctx.encoder2, ctx.encoder_ppBuffer_in2)) != OMX_ErrorNone) {
                    omx_die(r, "Failed to request emptying of the input buffer on encoder input port 200");
                }
            }
        }
        ...
        if(ctx.encoder_output_buffer_available2) {
            if(ctx.encoder_ppBuffer_out2->nFlags & OMX_BUFFERFLAG_ENDOFFRAME) {
                frame_out2++;
            }
            // Flush buffer to output file
            ...
            say("Read from output buffer and wrote to output file %d/%d, frame %d", ctx.encoder_ppBuffer_out2->nFilledLen, ctx.encoder_ppBuffer_out2->nAllocLen, frame_out2 + 1);
        }
        if(ctx.encoder_output_buffer_available2 || !frame_out2) {
            // Buffer flushed, request a new buffer to be filled by the encoder component
        ...
        if(want_quit && frame_out == frame_in){ //){ //){ //){ //){ //){ //){ //){ // && frame_out2 == frame_in) {  // 모든 프레임을 마무리했음.
            break;
        }
        // Would be better to use signaling here but hey this works too
        usleep(10);  // busy-while에 대한 CPU부하를 감쇄함. 
    } // While-loop 
    say("Cleaning up...");
    ...
    // 3.1 Flush the buffers on each component
    // we can share flush event flag because it only used one by one!
    // 1st 
    ...
    block_until_flushed(&ctx);
    // 2nd 
    if((r = OMX_SendCommand(ctx.encoder2, OMX_CommandFlush, 200, NULL)) != OMX_ErrorNone) {
        omx_die(r, "Failed to flush buffers of encoder input port 200");
    }
    block_until_flushed(&ctx);
    if((r = OMX_SendCommand(ctx.encoder2, OMX_CommandFlush, 201, NULL)) != OMX_ErrorNone) {
        omx_die(r, "Failed to flush buffers of encoder output port 201");
    }
    block_until_flushed(&ctx);

    // 3.2 Disable all the ports
    // 1st 
    ...
    // 2nd 
    if((r = OMX_SendCommand(ctx.encoder2, OMX_CommandPortDisable, 200, NULL)) != OMX_ErrorNone) {
        omx_die(r, "Failed to disable encoder input port 200");
    }
    block_until_port_changed(ctx.encoder2, 200, OMX_FALSE);
    if((r = OMX_SendCommand(ctx.encoder, OMX_CommandPortDisable, 201, NULL)) != OMX_ErrorNone) {
        omx_die(r, "Failed to disable encoder output port 201");
    }
    block_until_port_changed(ctx.encoder2, 201, OMX_FALSE);

    // 3.3 Free all the buffers
    // 1st
    ...
    // 2nd 
    if((r = OMX_FreeBuffer(ctx.encoder2, 200, ctx.encoder_ppBuffer_in)) != OMX_ErrorNone) {
        omx_die(r, "Failed to free buffer for encoder input port 200");
    }
    if((r = OMX_FreeBuffer(ctx.encoder2, 201, ctx.encoder_ppBuffer_out)) != OMX_ErrorNone) {
        omx_die(r, "Failed to free buffer for encoder output port 201");
    }

    // 3.4 Transition all the components to idle and then to loaded states
    // 1st 
    if((r = OMX_SendCommand(ctx.encoder, OMX_CommandStateSet, OMX_StateIdle, NULL)) != OMX_ErrorNone) {
        omx_die(r, "Failed to switch state of the encoder component to idle");
    }
    ...
    block_until_state_changed(ctx.encoder2, OMX_StateIdle);
    if((r = OMX_SendCommand(ctx.encoder2, OMX_CommandStateSet, OMX_StateLoaded, NULL)) != OMX_ErrorNone) {
        omx_die(r, "Failed to switch state of the encoder component to loaded");
    }
    block_until_state_changed(ctx.encoder2, OMX_StateLoaded);
    ...
    // 2nd 
    if((r = OMX_FreeHandle(ctx.encoder2)) != OMX_ErrorNone) {
        omx_die(r, "Failed to free encoder component handle");
    }
